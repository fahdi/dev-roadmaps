...
| [RAG vs CAG](artifacts/rag-vs-cag.html) | Interactive. Corpus size against context window decides whether cache-augmented generation is available at all, and how often the corpus changes decides whether it is wise. Retrieval works at any size and owns the risk of missing the passage |
| [Pipeline Failure Autopilot](artifacts/incident-autopilot.html) | Interactive. Four diagnostic agents, five failure modes and an idempotency gate. Only a transient failure on a safely rerunnable task auto-recovers; everything else escalates with the evidence already attached |
| [Containers: What Docker Actually Skips](artifacts/what-docker-skips.html) | Interactive. The VM boot stack against a process with two sets of kernel switches flipped. Toggle namespaces and cgroups off individually and the container degrades into an ordinary process, which is what it was all along |
| [Pod vs Container](artifacts/pod-vs-container.html) | Interactive. Add sidecars to a pod and see what one shared network namespace costs: two containers cannot both bind 8080, and scaling a Deployment multiplies every sidecar along with the service |
